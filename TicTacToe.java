  ...
  public int move() {
    ...
    while (!valid_input) {
      ...
      if (Character.isDigit(move_str.toCharArray()[0])) {
        //gets user input, parse it to Int
        move_int = Integer.parseInt(move_str);
        //check if the given position is in
...
// 7|8|9
  ...
  public TicTacToe() {
    ...
    board = new int[N][N];
    //initialize all the squares to 0
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < N; j++) {
        board[i][j] = 0;
      }
  ...
  //check if someone wins, looking by row, column and diags if
  //the first square is != 0 and if there are 3 consecutive squares with 1 or 2
